...
(function () {
  ...
  const DICT = {
    ...
    adm_pool_n: { bn: "একসাথে", en: "at once" },
  ...
  function applyLang(l) {
    ...
    if (self.APP.crm) self.APP.crm.render();
  ...
  // ---------- CSV / Excel import ----------
  /* The pure import readers — parseCSV, detectCols, swapCheck — moved to lib/import.js (self.APP.imp),
     the same way lib/xlsx.js took the pure workbook engine. Thin references kept here for the import
     orchestration below (applyImported) and for the menus. The orchestration itself stays: it writes
     `entries` and reads importSrc/srcBase, which the checkpoint writes too. */
  const parseCSV = self.APP.imp.parseCSV;
  const detectCols = self.APP.imp.detectCols;
  const swapCheck = self.APP.imp.swapCheck;
  ...
  /* The xlsx/zip read side (inflateRaw, unzip, colIdx, RELS_NS, sheetsOf, shared, sheet, readXlsx)
     moved to lib/xlsx.js — self.XLSX. Pure and stateless, so it reads a workbook the same whoever
     calls it. readXlsx is still called from loadXlsx below and handed to the menus through
     self.APP, so it keeps a thin reference; the rest are only reached through self.XLSX. */
  const readXlsx = self.XLSX.readXlsx;
  ...
  /* The xlsx/zip write side (CRC, crc32Run, crc32, cat, xesc, cl, FILL_STYLE, LINK_STYLE, rowXml,
     sheetChunks, sheetXml, ctXml, tabName, wbXml, wbrXml, deflateRaw, deflateChunks, zipPack,
     zipStore) moved to lib/xlsx.js — self.XLSX. xesc is still used by the HTML report below, so it
     keeps a thin reference; the sheet/zip helpers that buildBook calls keep theirs further down. */
  const xesc = self.XLSX.xesc;
  ...
  /* rowXml, sheetChunks, sheetXml, ctXml, tabName, wbXml, wbrXml moved to lib/xlsx.js (self.XLSX).
     buildBook below writes worksheets through sheetChunks and names the parts through ctXml/wbXml/
     wbrXml, so those four keep thin references; rowXml/sheetXml/tabName are reached via self.XLSX. */
  const sheetChunks = self.XLSX.sheetChunks;
  const ctXml = self.XLSX.ctXml, wbXml = self.XLSX.wbXml, wbrXml = self.XLSX.wbrXml;
  ...
  /* deflateRaw, deflateChunks, zipPack, zipStore moved to lib/xlsx.js (self.XLSX). buildBook packs
     the workbook through zipPack, so it keeps a thin reference; the rest are reached via self.XLSX. */
  const zipPack = self.XLSX.zipPack;
  ...
  let crmTab = "dash";
  /* Debounce timers for the New Admission institute/approver typeaheads. The handlers live in
     js/adm.js; only these wire-owned timers stay here, where the input listeners are attached. */
  let admInstTimer = null, admApprTimer = null;
  ...
  function wire() {
    ...
    if ($("admPar")) $("admPar").addEventListener("click", function () { self.APP.adm.setMode("parallel"); });
    if ($("admHttp")) $("admHttp").addEventListener("click", function () { self.APP.adm.setRunMode("http"); });
    if ($("admBrowserBtn")) $("admBrowserBtn").addEventListener("click", function () { self.APP.adm.setRunMode("browser"); });
    if ($("admHeadless")) $("admHeadless").addEventListener("click", function () { self.APP.adm.setRunMode("headless"); });
    if ($("admLoad")) $("admLoad").addEventListener("click", self.APP.adm.loadForm);
    if ($("admProgram")) $("admProgram").addEventListener("change", self.APP.adm.onProgram);
    if ($("admSession")) $("admSession").addEventListener("change", self.APP.adm.onSession);
    if ($("admBranch")) $("admBranch").addEventListener("change", self.APP.adm.onBranch);
    if ($("admGender")) $("admGender").addEventListener("change", function () { self.APP.adm.onSession(); });   // branch depends on gender; onSession no-ops until the form is loaded
    if ($("admInst")) $("admInst").addEventListener("input", function () { if (admInstTimer) clearTimeout(admInstTimer); admInstTimer = setTimeout(self.APP.adm.instSearch, 350); });
    if ($("admApprover")) $("admApprover").addEventListener("input", function () { if (admApprTimer) clearTimeout(admApprTimer); admApprTimer = setTimeout(self.APP.adm.apprSearch, 350); });
    self.APP.adm.setMode("sequential");
    if ($("admBase")) $("admBase").addEventListener("input", function () { try { chrome.storage.local.set({ admBase: this.value }); } catch (e) {} self.APP.adm.connDebounced(); });
    ["crmBase", "crmCount", "crmUsers"].forEach(function (id) { const e = $(id); if (e) e.addEventListener("input", self.APP.crm.render); });
    if ($("crmBase")) $("crmBase").addEventListener("input", function () { try { chrome.storage.local.set({ crmBase: this.value }); } catch (e) {} self.APP.crm.connDebounced(); });
    if ($("crmUsers")) $("crmUsers").addEventListener("input", self.APP.crm.nextLabel);
    if ($("crmHeaded")) $("crmHeaded").addEventListener("change", self.APP.crm.render);
    if ($("crmClose")) $("crmClose").addEventListener("change", self.APP.crm.render);
    if ($("crmMax")) $("crmMax").addEventListener("change", self.APP.crm.render);
    if ($("crmFile")) $("crmFile").addEventListener("change", function (ev) { const f = ev.target.files && ev.target.files[0]; if (f) self.APP.crm.importFile(f); ev.target.value = ""; });
    if ($("crmLinkBtn")) $("crmLinkBtn").addEventListener("click", self.APP.crm.importSheet);
    if ($("crmLink")) $("crmLink").addEventListener("keydown", function (e) { if (e.key === "Enter") self.APP.crm.importSheet(); });
    if ($("crmDl")) $("crmDl").addEventListener("click", self.APP.crm.download);
    if ($("crmClear")) $("crmClear").addEventListener("click", self.APP.crm.clear);
    if ($("crmRun")) $("crmRun").addEventListener("click", self.APP.crm.run);
    if ($("crmStop")) $("crmStop").addEventListener("click", self.APP.crm.stop);
    if ($("crmPar")) $("crmPar").addEventListener("click", function () { self.APP.crm.setMode("parallel"); });
    if ($("crmSeq")) $("crmSeq").addEventListener("click", function () { self.APP.crm.setMode("sequential"); });
  ...
  /* Hand the split-out menus (js/adm.js, js/crm.js) the core helpers and state they read through
     self.APP. Runs before wire()/applyLang()/showPage() below, so every menu call resolves. */
  self.APP = self.APP || {};
  Object.assign(self.APP, {
    $: $, t: t, sleep: sleep, fetchHtml: fetchHtml, parseCSV: parseCSV, readXlsx: readXlsx,
